...
```{r, echo = FALSE}
## IMPORTANT: Compile comp-literacy FIRST!!!!!
```
...
woRkshoptools::include_picture("./fig_git-dot-folder.png")
```
...
woRkshoptools::include_picture("./fig_git-bash-version.png")
```
...
woRkshoptools::include_picture("./fig_git-workflow-hahm.png")
```

.small[(Source: https://teaching.dahahm.de/teaching/ss22/dissys/2022/05/31/git_workflow.html)]
...
### Some things do NOT belong in your Git repo

- Binary files do not belong in your repository

- Also, sensitive data cannot go into your repo (especially, if you push it to
a remote location)
...
woRkshoptools::include_picture("./fig_git-dag.png")
```
...
```{r, out.width = "100%", echo = F}
woRkshoptools::include_picture("./fig_branches1.png")
```
...
### How to clone a remote repository I
...
- Use the git command `git remote add <name> <url>`. 

- The usual name for `<name>` is `origin`, however, feel free to choose another 
name. 

- The `<url>` part also determines the authentication method (`git@git...`: ssh 
and `https://github.com/...`: https) 

- Two examples: 
  - `git@git.gesis.org:weissbd/ps2017-xx-intro2git.git`
  - `https://github.com/berndweiss/tuma.git`
