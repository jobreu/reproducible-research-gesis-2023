...
plot_dependency_graph(file.path(.libPaths(), "usethis"))
```
...
- `renv` works with more repositories than does groundhog 
...
- When talking about a "reproducible R environment", this mostly refers to all
the R packages that are being used

- Most of us have *one* "system library", where all installed R packages can be 
found (mine can be found here: `r .libPaths()`)
...
well-defined format, created to add specific functionality" [1]
...
## Is one system library a good idea?

However, having *one* central system library is really bad, especially when 
things break -- and things will break! ("dependency hell")
...
renv introduces "project libraries"
...
- renv creates a R project-related library and installs packages into that 
specific library
...
working well (meaning: in terms of "reproducible R environment") 
...
renv uses this file to configure your R session to use the project library
...
all you need to commit into your Git repository
...
So what? Use `install.packages()` or `renv::install()`
